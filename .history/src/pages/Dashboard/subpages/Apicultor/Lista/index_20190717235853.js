...
class ListApicultor extends React.Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              <Button type="default" shape="circle" icon="form" size="default" idUser={record.id} onClick={() => this.handleOpenModalEdit(record)} />
              <Popconfirm title="Deletar apicultor?" okText="Sim" cancelText="Cancelar"
                onConfirm={() => this.handleDelete(record.id)}>
                <Button type="default" shape="circle" icon="delete" size="default" />
              </Popconfirm>
            </div>
          );
        }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.code === 204) {
      ...
      pagination.pageSize = nextProps.list_apicultor && nextProps.list_apicultor.per_page ? nextProps.list_apicultor.per_page : 1;
