...
class ListApicultor extends React.Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              <Button type="default" shape="circle" icon="form" size="large" idUser={record.id} onClick={ () => this.handleOpenModalEdit(record.id) } />              
  ...
  handleOpenModalEdit = (id) => {
    this.setState( { idUser: id, visibleModalEdit: true});
  }
  ...
  render() {
    ...
    return (
      ...
      <Layout.Content style={{ margin: '0 16px' }}>
        ...
        <ModalEditApicultor idUser={this.state.idUser} visible={visibleModalEdit} onClose={() => this.setState({ visibleModalEdit: false })}
