...
class ListApicultor extends React.Component {
  ...
  render() {
    ...
    let apicultores = this.props.list_apicultor || [];
    ...
    const columns = [
      ...
      },
      {
        ...
        render: apicultores => {
          return (
            <span>
              {/* <Tag color="blue" key={'/qtd'}>{this.props.}</Tag> */}
              {apicultores.map(apicultor => <Tag color="blue" key={apicultor.id}>{apicultor.qtdApiarios}</Tag>)}
            </span>
          );
        }
      },
