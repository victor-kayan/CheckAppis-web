...
class ModalNovoApicultor extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.code === 201) {
      ...
      this.setState({ alertVisible: true, message: 'Usuário criado com sucesso', tentouSalvar: true, typeAlert: 'success' });
      //this.refs.alert.openAlert();
    } else if(nextProps.tentouSalvar){
      this.setState({
        alertVisible: true, message: 'Infelizmente houve algum problema',
        tentouSalvar: true, typeAlert: 'error'
      });
     // this.refs.openAlert.openAlert();
    }
    this.setState({ tentouSalvar: false});
  }
      ...
      <div>
        <Modal
          ...
          <AlertComponent visible={alertVisible} onClose={ () => this.setState( { alertVisible: false })} 
            tentouSalvar={this.state.tentouSalvar} type={typeAlert} message={message} />
