...
class ModalNovoApicultor extends React.Component {
  ...
  // handleOk = () => {
  //   this.refs.formCabo.validateFields((err, values) => {
  //     if (!err) {
  //       const { edit, cabo } = this.state;

  //       if (edit) {
  //         this.props.actions.cabo.update({
  //           cabo_id: cabo && cabo.id,
  //           ...values
  //         });
  ...
  //       this.props.actions.cabo.create({ ...values });
  //       this.setState({ done: true });
  //     }
  //   });
  // };
    ...
    return (
      ...
      >
        <AlertComponent
          ...
          message={message}
        />
        <FormaNovoApicultor ref="formNovoApicultor" {...this.props}  />;
      </Modal>
    );
  }
}
