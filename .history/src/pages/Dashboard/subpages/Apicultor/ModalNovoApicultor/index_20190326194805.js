...
class ModalNovoApicultor extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.code === 201) {
      this.setState({ alertVisible: true, message: 'Usuário criado com sucesso', buttonClicou: true, typeAlert: 'success' });
    } else if(nextProps.code < 200 || nextProps.code >= 300  ) {
      this.setState({
        alertVisible: true, message: 'Infelizmente houve algum problema', buttonClicou: true, typeAlert: 'error'
      });
      console.log(this.refs.alert.openAlert());
    }
